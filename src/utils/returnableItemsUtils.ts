...
export const calculateReturnableItemsWithDates = (clientTransactions: CreditTransaction[]): {text: string, date: string, time: string}[] => {
  ...
  clientTransactions.forEach(transaction => {
    ...
    // REMOVED the duplicate processing to prevent double counting
  });
...
/**
 * Gets returnable items for a client with quantities only
 * @param clientTransactions - Array of client transactions
 * @returns Object mapping item type to quantity
 */
export const getReturnableItems = (clientTransactions: CreditTransaction[]): {[key: string]: number} => {
  const returnableItems: {[key: string]: number} = {};
