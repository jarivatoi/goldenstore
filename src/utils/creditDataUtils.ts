...
import { initReturnsDB } from './returnsManager';
...
export const exportCompleteDatabase = async (notifications?: NotificationCallbacks): Promise<void> => {
  try {
    ...
    const returnsData = localStorage.getItem('order_returns'); // For backward compatibility
    ...
    const exportData = {
      ...
      // Returns data (for backward compatibility - will be migrated to IndexedDB)
      returns: returnsData ? JSON.parse(returnsData) : []
    };
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (confirmImport) {
            ...
            await initReturnsDB(); // Initialize returns IndexedDB
            ...
            // Import returns data (from localStorage backup or new IndexedDB format)
            if (data.returns && data.returns.length > 0) {
              await appDBManager.importReturns(data.returns);
            }

            if (notifications) {
              notifications.showAlert({ type: 'success', message: `Successfully imported complete Golden Store database!

• Returns: ${data.returns?.length || 0} returns imported
