...
export const CreditProvider: React.FC<CreditProviderProps> = ({ children }) => {
  ...
  // Memoize the sorted clients list to avoid re-sorting on every search
  ...
  // Search clients - MEMOIZED to prevent unnecessary recalculations
  const searchClients = useCallback((query: string): Client[] => {
    if (!query.trim()) {
      // Return pre-sorted clients when no search query
      return sortedClients;
    }

    // Check if query ends with "?" (starts-with filter)
    if (query.trim().endsWith('?')) {
      const searchTerm = query.trim().slice(0, -1).toLowerCase();

      if (searchTerm.length > 0) {
        return clients.filter(client =>
          client.name.toLowerCase().startsWith(searchTerm)
        ).sort((a, b) => {
  ...
  }, [sortedClients, clients, transactions]);
  ...
  const addTransaction = async (client: Client, description: string, amount: number) => {
    try {
      ...
      // Update transactions state AND client's total debt synchronously using flushSync
      flushSync(() => {
  ...
  const addBatchTransactions = async (client: Client, transactionData: Array<{description: string, amount: number}>) => {
    try {
      ...
      // Save only the updated client to IndexedDB (faster than saving all)
      if (updatedClient) {
        await creditDBManager.saveClient({
          ...updatedClient,
          createdAt: updatedClient.createdAt.toISOString(),
          lastTransactionAt: updatedClient.lastTransactionAt.toISOString()
        });
      }
    ...
    try {
      ...
      const transactionsToKeep = transactions.filter(transaction =>
        transaction.clientId !== clientId ? true : (
          ...
          transaction.description.toLowerCase().includes('caisse')
        )
      );
      ...
      // Save to IndexedDB - delete individual transactions instead of saveAll
  ...
  const settleClientWithFullClear = async (clientId: string) => {
    try {
      ...
      // Delete from IndexedDB using efficient bulk delete methods
      await creditDBManager.deleteTransactionsByClient(clientId);
      await creditDBManager.deletePaymentsByClient(clientId);

      // Save new payment and updated client
      await creditDBManager.savePayment({
        ...newPayment,
        date: newPayment.date.toISOString()
      });
