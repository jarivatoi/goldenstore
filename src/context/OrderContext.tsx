...
import { OrderCategory, OrderItemTemplate, Order, OrderItem, OrderReturn } from '../types';
...
interface OrderContextType {
  ...
  addReturnToOrder?: (orderId: string, returnData: Omit<OrderReturn, 'id' | 'createdAt'>) => Promise<void>;
 ...
 * FORMAT NAME UTILITY
 * ===================
 * Formats item name to ensure consistent display with auto-capitalization after spaces, parentheses, and slashes
 * Also preserves unit notation (e.g., "2l" -> "2L")
 */
const formatName = (name: string): string => {
  ...
  // Then apply unit preservation and standard capitalization
  return capitalized
    .split(' ')
    .map(word => {
...
export const OrderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      try {
        ...
        if (supabase) {
          try {
            ...
            const storedCategories = await appDBManager.getOrderCategories();
            const transformedCategories: OrderCategory[] = storedCategories ? storedCategories.map((category: any) => ({
              ...category,
              createdAt: new Date(category.createdAt)
            })) : [];
            
            await appDBManager.initDB();
            const storedTemplates = await appDBManager.getOrderTemplates();
            const transformedTemplates: OrderItemTemplate[] = storedTemplates ? storedTemplates.map((template: any) => ({
              ...template,
            ...
            await appDBManager.initDB();
            const storedOrders = await appDBManager.getOrders();
            const transformedOrders: Order[] = storedOrders ? storedOrders.map((order: any) => ({
              ...order,
          ...
          // Fallback to IndexedDB when Supabase is not available
          console.log('Using IndexedDB for order data');
          await appDBManager.initDB();
          const storedCategories = await appDBManager.getOrderCategories();
          const transformedCategories: OrderCategory[] = storedCategories ? storedCategories.map((category: any) => ({
            ...category,
  ...
  const addCategory = async (name: string, vatPercentage: number = 15): Promise<OrderCategory> => {
    try {
      // Use the name as-is without any formatting
    ...
    try {
      ...
      if (supabase) {
        ...
        setCategories(prev => prev.map(cat => 
          cat.id === id ? { ...cat, name: categoryName, vatPercentage } : cat
        ));
      } else {
        // Fallback to localStorage
  ...
  const addOrder = async (categoryId: string, orderDate: Date, items: OrderItem[]): Promise<Order> => {
    try {
      ...
      const newOrder: Order = {
        ...
        orderDate,
        items,
        returns: [], // Initialize empty returns array
        totalCost,
        createdAt: new Date()
      };
