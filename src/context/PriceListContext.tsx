...
export const PriceListProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateItem = async (id: string, name: string, price: number, grossPrice?: number) => {
    try {
      ...
      // Always use the provided grossPrice value (even if 0)
      // Ensure no NaN values
      const finalGrossPrice = isNaN(grossPrice ?? 0) ? 0 : (grossPrice ?? 0);

      // Check if the item has actually changed
      // Only update lastEditedAt if name or price changes, or if grossPrice changes to a non-zero value
      const hasChanged = existingItem.name !== capitalizedName ||
                         existingItem.price !== price ||
                         (existingItem.grossPrice !== finalGrossPrice && finalGrossPrice !== 0);
