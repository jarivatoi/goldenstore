            ...
            <div className="mt-4 select-none">
              <div className="relative w-full max-w-md mx-auto select-none">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-4 py-3 text-lg border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 select-text"
                />
              </div>
            </div>
          </>
...
const CreateOrderModal: React.FC<CreateOrderModalProps> = ({ category, itemTemplates, onClose, onAdd }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsSubmitting(true);
      await onAdd(category.id, new Date(orderDate), orderItems);
      onClose();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 select-none">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-hidden select-none">
        <div className="p-6 border-b select-none">
          <div className="flex justify-between items-center select-none">
            <h3 className="text-lg font-semibold text-gray-900 select-none">
              Create Order for {category.name}
            </h3>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)] select-none">
          <form onSubmit={handleSubmit} className="select-none">
            <div className="mb-6 select-none">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 select-text"
                required
              />
            </div>
            ...
            {availableTemplates.length > 0 && (
              <div className="mb-6 select-none">
                <h4 className="text-sm font-medium text-gray-700 mb-3 select-none">Add Items to Order</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 select-none">
                  {availableTemplates.map((template) => (
                    <button
                      ...
                      type="button"
                      onClick={() => addItemToOrder(template)}
                      className="flex items-center justify-between p-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors text-left select-none"
                    >
                      <span className="font-medium text-gray-800 select-none">{template.name}</span>
                      <span className="text-sm text-gray-600 select-none">Rs {template.unitPrice.toFixed(2)}</span>
                    </button>
            ...
            {orderItems.length > 0 && (
              <div className="mb-6 select-none">
                ...
                <div className="space-y-2 select-none">
                  {orderItems.map((item) => (
                    <div key={item.id} className="flex items-center justify-between text-sm bg-gray-50 p-3 rounded-lg select-none">
                      <div className="flex items-center gap-3 select-none">
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            ...
                            onChange={(e) => updateOrderItem(item.id, 'quantity', parseInt(e.target.value) || 0)}
                            onFocus={(e) => e.target.select()}
                            className="w-16 px-2 py-1 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500 placeholder-gray-400 select-text"
                            placeholder="0"
                            style={{ textAlign: 'center' }}
                            min="0"
                          />
                        ...
                        <span className={`select-none ${item.isAvailable ? '' : 'line-through text-gray-400'}`}>
                          {getItemTemplateName(item.templateId)} @{item.unitPrice.toFixed(2)} + {item.isVatNil ? 'VAT Nil' : `VAT(Rs ${item.vatAmount.toFixed(2)})`}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 select-none">
            ...
            {/* Total */}
            {orderItems.length > 0 && (
              <div className="border-t pt-4 mb-6 select-none">
                <div className="flex justify-between items-center select-none">
                  <span className="font-semibold text-gray-800 select-none">Total Amount:</span>
                  <span className="font-bold text-green-600 text-lg select-none">Rs {totalCost.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
...
const EditOrderModal: React.FC<EditOrderModalProps> = ({ order, itemTemplates, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsSubmitting(true);
      await onUpdate(order.id, new Date(orderDate), orderItems);
      onClose();
    } catch (error) {
