...
const ClientSearchModal: React.FC<ClientSearchModalProps> = ({ 
  ...
  const descriptionInputRef = useRef<HTMLInputElement>(null); // Reference to description input
  ...
  const handleAddNewClient = async () => {
    // Remove focus from description input to dismiss keyboard
    if (descriptionInputRef.current) {
      descriptionInputRef.current.blur();
    }

    // Small delay to ensure keyboard dismisses smoothly
        ...
        <div className="overflow-y-auto flex-1 select-none p-6">
          ...
          <div className="mb-4 select-none">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Capitalize each word properly, including brands starting with numbers and text after (
                let formatted = '';
                const words = value.split(/(\s+|,|\/)/); // Split but keep delimiters

                for (let i = 0; i < words.length; i++) {
                  const word = words[i];
                  // Check if this is a delimiter (space, comma, or /)
                  if (/^\s+$/.test(word) || word === ',' || word === '/') {
                    formatted += word;
                  } else if (word.length > 0) {
                    // Check if word is a unit measure (e.g., "1L", "1.5L", "2L") - preserve uppercase L
                    if (/^\d+\.?\d*[lL]$/i.test(word)) {
                      // Format as number + uppercase L (e.g., "1l" → "1L", "1.5l" → "1.5L")
                      formatted += word.slice(0, -1) + 'L';
                    }
                    // Check if word contains opening parenthesis
                    else if (word.includes('(')) {
                      // Split by parenthesis and handle each part
                      ...
                      for (let j = 0; j < parts.length; j++) {
                        ...
                        if (j > 0) {
                          // Add back the opening parenthesis
                          processedWord += '(';
                        }

                        if (part.length > 0) {
                          // Check if part is a unit measure
                          if (/^\d+\.?\d*[lL]$/i.test(part)) {
                            processedWord += part.slice(0, -1) + 'L';
                          }
                          // Process the part (could be before or after parenthesis)
                          else if (/^\d/.test(part)) {
                            // Part starts with digit(s)
                            const match = part.match(/^(\d+)(.*)$/);
                            if (match && match[2]) {
                              const digits = match[1];
                              const letters = match[2];
                              processedWord += digits + letters.charAt(0).toUpperCase() + letters.slice(1).toLowerCase();
                            } else {
                              processedWord += part;
                            }
                          } else {
                            // Regular text: capitalize first letter, lowercase rest
                            processedWord += part.charAt(0).toUpperCase() + part.slice(1).toLowerCase();
                          }
                        }
                      }

                      formatted += processedWord;
                    } else if (/^\d/.test(word)) {
                      // If word starts with digit(s), capitalize first letter after digits
                      // e.g., "7seas" → "7Seas", "7Seas" → "7Seas"
                      const match = word.match(/^(\d+)(.*)$/);
                      if (match && match[2]) {
                        const digits = match[1];
                        const letters = match[2];
                        // Capitalize first letter after digits, lowercase the rest
                        formatted += digits + letters.charAt(0).toUpperCase() + letters.slice(1).toLowerCase();
                      } else {
                        formatted += word;
                      }
                    } else {
                ...
                const validationWords = formatted.split(/[\s,]+/).filter(word => word.trim());

                // Get the last two words
                if (validationWords.length >= 2) {
                  const lastWord = validationWords[validationWords.length - 1];
                  const secondLastWord = validationWords[validationWords.length - 2];
          ...
          <div className="mb-4 select-none">
            ...
            <div className="grid grid-cols-3 gap-2 mb-3 select-none">
              {['Foodstuffs', 'Cig', 'Rum', 'Can', 'Soft Drinks', 'Cakes', 'Chopine', 'Bouteille', 'Bouteille 1L', 'Bouteille 1.5L', 'Caisse', 'Caisse (Ch)'].map((action) => (
                <button
                  key={action}
                  type="button"
            ...
            <div className="grid grid-cols-3 gap-2 select-none">
              ...
              <button
                type="button"
                onClick={() => handleQuickAction('Juice Payment')}
                className="px-3 py-2 text-sm bg-blue-100 hover:bg-blue-200 text-blue-800 rounded-lg transition-colors select-none"
              >
                Juice Payment
              </button>
