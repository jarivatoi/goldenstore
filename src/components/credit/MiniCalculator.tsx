...
import { KeypadHandler, KeypadState } from '../../calculator/KeypadHandler';
...
const MiniCalculator: React.FC<MiniCalculatorProps> = ({
  ...
  const [calculatorState, setCalculatorState] = useState<KeypadState>(KeypadHandler.initializeState());
  ...
  // Use KeypadHandler for calculator logic
  const handleCalculatorInput = (input: string) => {
    // Map mini calculator buttons to KeypadHandler inputs
    let mappedInput = input;

    // Map backspace symbol to arrow
    if (input === '⌫') {
      mappedInput = '→';
    }

    // Map operators to KeypadHandler format
    if (input === '*') {
      mappedInput = '×';
    } else if (input === '/') {
      mappedInput = '÷';
    ...
    // Handle memory buttons - KeypadHandler uses different names
    if (input === 'MR') {
      mappedInput = 'MRC';
    }

    // MC - clear memory by setting it to 0 directly
    if (input === 'MC') {
      setCalculatorState({
        ...calculatorState,
        memory: 0
    ...
    // Process input through KeypadHandler
    const result = KeypadHandler.processInput(calculatorState, mappedInput);
    setCalculatorState(result.state);
  };
  ...
  const modalContent = isVisible ? (
    ...
    >
      ...
      >
        <div className="bg-gray-100 rounded-lg p-3 text-right relative">
          ...
          <div className="text-lg font-mono text-gray-800 min-h-[1.5rem] flex items-center justify-end overflow-hidden">
            <div className="truncate max-w-full" title={calculatorState.display}>
              {formatCalculatorValue(calculatorState.display)}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-4 gap-1 mb-3">
          ...
          <button
            ...
            onTouchEnd={(e) => {
              e.stopPropagation();
              e.preventDefault();
              if (calculatorState.display !== 'Error') {
                setShowClientSearch(true);
              }
            }}
            disabled={calculatorState.display === 'Error'}
            onClick={(e) => {
              e.stopPropagation();
              e.preventDefault();
              if (calculatorState.display !== 'Error') {
      ...
      {showClientSearch && (
        <ClientSearchModal
          calculatorValue={calculatorState.display}
          onClose={() => {
          ...
          onAddToClient={async (client, description) => {
            try {
              ...
              // Reset calculator
              setCalculatorState(KeypadHandler.initializeState());
              setShowClientSearch(false);
              setError('');
            } catch (err) {
