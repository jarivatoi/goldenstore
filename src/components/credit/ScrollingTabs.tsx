...
const ScrollingTabs: React.FC<ScrollingTabsProps> = ({
  ...
  const [visibleClientIds, setVisibleClientIds] = React.useState<Set<string>>(new Set());
  const [virtualizedClients, setVirtualizedClients] = React.useState<Client[]>([]);
  const cardWidth = 160 + 24; // 160px width + 24px gap
  ...
  // Memoize returnable items calculations only for virtualized clients
  const clientReturnableItemsMap = useMemo(() => {
    const map = new Map<string, string>();

    // Only calculate for virtualized clients to save performance
    const clientsToCalculate = virtualizedClients.length > 0 ? virtualizedClients : stableSortedClients;

    clientsToCalculate.forEach(client => {
      ...
      const truncatedItems = fullReturnableItems.map(itemText => {
        const quantityMatch = itemText.match(/^(\d+)/);
        const quantity = quantityMatch ? quantityMatch[1] : '';

        if (itemText.includes('Chopine')) return `${quantity} (Ch)`;
        if (itemText.includes('Caisse')) return `${quantity} (Cs)`;

        const sizeMatch = itemText.match(/(\d+(?:\.\d+)?L)\b/i);
        if (sizeMatch) return `${quantity} (${sizeMatch[1]})`;

        if (itemText.includes('Bouteille')) return `${quantity} (Bt)`;

        return itemText.substring(0, 10);
      });

      map.set(client.id, truncatedItems.join(', '));
    });

    return map;
  }, [virtualizedClients, stableSortedClients, getTransactions, forceUpdate]);
  ...
  const setupContinuousScroll = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      // Use a slower, fixed speed for better performance on mobile
      const pixelsPerSecond = 60; // Increased from 40 for faster scrolling
      const calculatedDuration = totalDistance / pixelsPerSecond;

      // Set minimum duration to ensure smooth animation even with few clients
      const minDuration = 10; // Increased from 8 seconds for smoother movement
      const duration = Math.max(calculatedDuration, minDuration);

      // Create seamless infinite timeline with GPU acceleration for better performance
  ...
  // Virtualization: Track and render only visible clients
  useEffect(() => {
    if (!contentRef.current || !containerRef.current) return;

    const updateVirtualizedClients = () => {
      const container = containerRef.current;
      const content = contentRef.current;
      ...
      // Only virtualize if we have 20+ clients
      if (stableSortedClients.length < 20) {
        // Render all clients without virtualization
        setVirtualizedClients(stableSortedClients);

        // Still track visible IDs for animations
        const allIds = new Set<string>(stableSortedClients.map(c => c.id));
        setVisibleClientIds(allIds);
        return;
      }

      // Calculate which indices are visible with buffer
      const buffer = 3; // Render 3 cards before and after visible area
      const visibleStartX = -currentX - (buffer * cardWidth);
      const visibleEndX = -currentX + containerWidth + (buffer * cardWidth);

      const startIndex = Math.max(0, Math.floor(visibleStartX / cardWidth));
      const endIndex = Math.min(
        stableSortedClients.length,
        Math.ceil(visibleEndX / cardWidth) + 1
      );

      // Get virtualized slice
      const newVirtualized = stableSortedClients.slice(startIndex, endIndex);

      // Track visible IDs for animations
      const newVisibleIds = new Set<string>();
      for (let i = startIndex; i < endIndex; i++) {
        if (stableSortedClients[i]) {
          newVisibleIds.add(stableSortedClients[i].id);
        }
      }

      // Only update if changed
      setVirtualizedClients(prev => {
        if (prev.length !== newVirtualized.length) return newVirtualized;
        for (let i = 0; i < newVirtualized.length; i++) {
          if (prev[i]?.id !== newVirtualized[i]?.id) return newVirtualized;
        }
        return prev;
      });

      setVisibleClientIds(prev => {
        if (prev.size !== newVisibleIds.size) return newVisibleIds;
        for (const id of newVisibleIds) {
          if (!prev.has(id)) return newVisibleIds;
        }
        return prev;
      });
    };
  ...
  return (
    ...
    <div ref={scrollingTabsRef} className="bg-white rounded-lg shadow-sm border border-gray-200 scrolling-tabs-component w-full" style={{ flexShrink: 0 }}>
      ...
      <div className="px-3 py-2">
        <div
          ref={containerRef}
          className={`overflow-hidden w-full h-30 flex items-end relative z-10 ${
            isBigCard ? 'overflow-x-auto' : ''
          }`}
          style={{
            height: '116px',
            overflow: 'hidden', // Explicitly clip bouncing animations
            paddingBottom: '4px',
        ...
        >
          <div
            ...
            style={{
              ...
              width: `${stableSortedClients.length * cardWidth}px`, // Set explicit width for proper scrolling
              // Remove snap behavior for big card
              ...(isBigCard && {
          ...
          >
            {stableSortedClients.map((client, index) => {
              // Check if this client should be rendered (virtualization)
              const isVirtualized = virtualizedClients.some(vc => vc.id === client.id);
              if (!isVirtualized && virtualizedClients.length > 0) {
                // Render placeholder for non-virtualized clients
                return (
                  <div
                    key={client.id}
              ...
              // Get pre-calculated returnable items from memoized map
              const currentReturnableItems = clientReturnableItemsMap.get(client.id) || '';

              // Calculate card background color inline
              const cardBackgroundColor = totalDebt <= 500
                ? 'bg-green-100 border-green-200'
                : totalDebt <= 1000
                  ? 'bg-orange-100 border-orange-200'
                  : 'bg-red-100 border-red-200';

              // Only animate if this card is entering or visible (not passed left edge)
              const isVisible = visibleClientIds.has(client.id);
              const shouldBounce = isVisible && totalDebt >= 1000;
              const shouldShake = isVisible && currentReturnableItems && recentlyUpdatedClient !== client.id;

              return (
                <div
                  key={client.id}
                  data-client-id={client.id}
                  className={`flex-shrink-0 px-4 py-2 rounded-lg border cursor-pointer w-40 flex items-center ${
                    isDragging
                      ? 'transition-none'
                    ...
                    isLinked
                      ...
                      : isDragging
                        ? cardBackgroundColor
                        : `${cardBackgroundColor} hover:shadow-md`
                  } ${
