                ...
                >
                  <div className="text-center">
                    ...
                    {clientFilter === 'returnables' ? (
                      ...
                      (() => {
                        // Check if client has both returnables and debt > 0
                        const clientTransactions = getClientTransactions(client.id);
                        const hasReturnables = clientTransactions.some(transaction => {
                          if (transaction.type === 'payment' || transaction.description.toLowerCase().includes('returned')) {
                            return false;
                          }
                          const description = transaction.description.toLowerCase();
                        ...
                        if (hasReturnables && totalDebt > 0) {
                          // Get returnable items text (same logic as above)
                          const returnableItems: {[key: string]: number} = {};
                          ...
                          const returnablesText = truncatedItems.join(', ');
                          const debtText = `Rs ${totalDebt.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                          
                          return (
                            <div className="relative">
                              {/* Returnables text - shows first */}
                              <div className="animate-text-swap text-orange-600">
                                {returnablesText}
                              </div>
                              {/* Debt amount - shows second (delayed) */}
                              <div 
                                className="absolute inset-0 animate-text-swap text-red-600"
                          ...
                          // Regular debt display for clients without returnables
                          return (
