  ...
  const setupContinuousScroll = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      draggableRef.current = Draggable.create(content, {
        type: "x",
        allowEventDefault: true, // Allow event default to enable touch events on child elements
        ...
        minimumMovement: 10, // Increase minimum movement to distinguish drag from tap (was 3)
              ...
              return (
                <div
                  ...
                  onMouseDown={(e) => {
                    ...
                    (e.target as any).mouseDownTime = Date.now();
                  }}
                  onMouseUp={(e) => {
                    // Handle mouse up to trigger click
                    const mouseDownTime = (e.target as any).mouseDownTime || 0;
                    const timeElapsed = Date.now() - mouseDownTime;
                    
                    // Only trigger click if mouse was pressed briefly
                    if (timeElapsed < 300) { // 300ms threshold for click
                      handleTabClick(client);
                    }
                  }}
                  onTouchStart={(e) => {
                    // Prevent drag when touching a tab
                    e.stopPropagation();
                    // Store touch start time for tap detection
                    (e.target as any).touchStartTime = Date.now();
                  }}
                  onTouchMove={(e) => {
                    // If we're moving significantly, prevent tab click
                    const touchStartTime = (e.target as any).touchStartTime || 0;
                    const timeElapsed = Date.now() - touchStartTime;
                    // If touch has moved and it's been less than 200ms, it's likely a scroll
                    if (timeElapsed < 200) {
                      e.preventDefault();
                    }
                  }}
                  onTouchEnd={(e) => {
                    // Handle touch end to trigger click
                    e.preventDefault();
                    const touchStartTime = (e.target as any).touchStartTime || 0;
                    const timeElapsed = Date.now() - touchStartTime;
                    
                    // Only trigger click if touch was brief (tap) and not a scroll
                    if (timeElapsed < 300) { // 300ms threshold for tap
                      handleTabClick(client);
                    }
                  }}
