...
const processItemReturn = async (itemType: string, returnQuantity: number) => {
  ...
  const { returnableItems, availableItems } = useMemo(() => {
    console.log('🔄 useMemo RECALCULATING availableItems...');
    console.log('📊 clientTransactions.length:', clientTransactions.length);

    const returnableItems = getReturnableItems();
    console.log('📦 returnableItems from getReturnableItems:', returnableItems);
    ...
    console.log('✅ availableItems after filtering (total > 0):', Object.keys(availableItems).length, 'items');
    console.log('📋 Available item types:', Object.keys(availableItems));
  ...
  const modalContent = (
    <div
      ...
      onTouchStart={(e) => {
        // Absorb any stray touch events on the overlay
        e.stopPropagation();
      }}
      onTouchEnd={(e) => {
        // Absorb any stray touch events on the overlay
    ...
    >
      <div
        className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto select-none"
        onTouchStart={(e) => {
                          ...
                          <h4 className="font-medium text-gray-800 select-none">
                            {(() => {
                              ...
                              if (itemType.includes('Bouteille') || itemType.includes('Chopine') || itemType.includes('Caisse')) {
                                // Already in correct format like "Bouteille 1.5L Pepsi", "Chopine 7Seas", or "Caisse Marlin"
                                ...
                                if (itemType.includes('Bouteille') || itemType.includes('Chopine') || itemType.includes('Caisse')) {
                                  ...
                                  for (let i = 0; i < parts.length; i++) {
                                    const part = parts[i];
                                    if (/\d+[Ll]$/.test(part) || part === 'Bouteille' || part === 'Chopine' || part === 'Caisse') {
                                      capitalizedParts.push(part);
                                      if (/\d+[Ll]$/.test(part)) foundSize = true;
                                    } else if (foundSize) {
                                      // This is a brand name after the size, capitalize it (including alphanumeric brands like "7Seas")
                                      if (/^\d/.test(part)) {
                                        ...
                                        if (match && match[2]) {
                                          ...
                                          capitalizedParts.push(digits + letters.charAt(0).toUpperCase() + letters.slice(1).toLowerCase());
                                        } else {
                                          capitalizedParts.push(part);
                                        }
                                      } else {
                                      ...
                                      // This is a brand name (could be after Chopine/Caisse without size, or before a size for Bouteille)
                                      ...
                                      if (/^\([A-Z][a-z]\)$/.test(part)) {
                                        ...
                                        // This is a brand name with numbers, capitalize first letter after digits
                                        const match = part.match(/^(\d+)(.*)$/);
                                        if (match && match[2]) {
                                          const digits = match[1];
                                          const letters = match[2];
                                        ...
                                        // This is a regular brand name, capitalize it
                                        capitalizedParts.push(part.charAt(0).toUpperCase() + part.slice(1).toLowerCase());
                                      }
                                    }
                                  }
                                ...
                                const parts = itemType.split(' ');
                                if (parts.length >= 2) {
                                  // Capitalize the brand name part (including alphanumeric brands like "7Seas")
                                ...
                                // Capitalize the brand name properly (including alphanumeric brands like "7Seas")
      ...
      {showSettleConfirm && settleAction && (
        <SettleConfirmationModal
          ...
          remainingItems={Object.entries(availableItems)
            ...
            .join(', ')}
          onConfirm={async () => {
            console.log('🚀 SETTLE ALL CONFIRM CLICKED');
            try {
              setIsProcessing(true);
              console.log('⚙️ isProcessing set to true');

              if (settleAction.type === 'all') {
                console.log('📋 settleAction.type is "all"');

                // Capture items upfront before processing to avoid stale data
                const itemEntries = Object.entries(availableItems);
                const itemCount = itemEntries.length;

                console.log(`🔄 Starting to process ${itemCount} item types for return`);
                console.log('📦 Items to process:', JSON.stringify(itemEntries.map(([type, data]) => ({ type, qty: data.total }))));
                console.log('👤 Client:', { id: client.id, name: client.name });

                // Prepare all transaction data for batch processing
                const transactionData = itemEntries.map(([itemType, data]) => {
                ...
                console.log(`📦 Batch processing ${transactionData.length} transactions...`);

                try {
                  // Process all returns in a single batch operation
                  await addBatchTransactions(client, transactionData);
                  console.log(`✅ Batch processing SUCCESSFUL for all items`);
                } catch (error) {
                  console.error(`❌ Batch processing FAILED:`, error);
                  throw error;
                }

                console.log(`✨ ALL ${itemCount} ITEMS PROCESSED IN BATCH`);

                // Show duplicate card for successful return settlement
                console.log('🎉 Dispatching showDuplicateCard event...');
                window.dispatchEvent(new CustomEvent('showDuplicateCard', {
                  detail: {
                    ...client,
                    isAccountClear: false,
