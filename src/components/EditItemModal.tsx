...
const EditItemModal: React.FC<EditItemModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If grossPrice is empty or 0, explicitly pass 0 (not undefined)
      const grossPriceValue = grossPrice ? parseFloat(grossPrice) : 0;
      const roundedGrossPrice = !isNaN(grossPriceValue) ? Math.round(grossPriceValue * 100) / 100 : 0;
      await onSave(
        item.id,
        name.trim(),
