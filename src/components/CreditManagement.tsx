...
const CreditManagement: React.FC = () => {
  ...
  const deferredMainGridSearchQuery = useDeferredValue(mainGridSearchQuery);

  // Track if we're in fast preview mode (showing G1-G5 only)
  const isShowingFastPreview = mainGridSearchQuery !== deferredMainGridSearchQuery && mainGridSearchQuery === '';
  ...
  useEffect(() => {
    ...
    const handleCheckNavigation = (event: CustomEvent) => {
      // Update display for CHECK navigation
      setAutoReplayDisplay(event.detail.displayValue);
    ...
    switch (clientFilter) {
      ...
      default:
        return baseClients.filter(client => {
          ...
          // Use the utility function to check for any returnable items (chopine, bouteille, caisse)
          if (hasUnreturnedItems(clientTransactions, client.name)) {
            return true;
          }

          // Exclude clients with zero debt and no returnables
          return false;
        });
    }
  ...
  // Memoize the base search result to avoid redundant searches
  const searchedClients = useMemo(() => {
    return searchClients(deferredMainGridSearchQuery);
  }, [deferredMainGridSearchQuery, searchClients]);

  // Fast preview: show G1-G5 instantly when clearing search
  const fastPreviewClients = useMemo(() => {
    // Sort clients numerically first
    const sortedClients = [...clients].sort((a, b) => {
      const aNum = parseInt(a.id.match(/G(\d+)/)?.[1] || '0');
      const bNum = parseInt(b.id.match(/G(\d+)/)?.[1] || '0');
      return aNum - bNum;
    });

    if (!showAllClients) {
      return sortedClients.filter(client => {
        const idMatch = client.id.match(/G(\d+)/);
        if (idMatch) {
    ...
    return sortedClients.slice(0, 5); // Just show first 5 for all clients mode
  }, [clients, showAllClients]);

  // Filter clients based on search - MEMOIZED for performance
  const filteredClients = useMemo(() => {
    let results;
    if (showAllClients) {
      results = searchedClients; // Show all clients when toggled - no filtering
    } else {
      // Only filter when NOT showing all clients
      results = searchedClients.filter(client => {
          const totalDebt = getClientTotalDebt(client.id);

          // Check if client has returnable items using the utility function
          const clientTransactions = getClientTransactions(client.id);
          const hasActualReturnableItems = hasUnreturnedItems(clientTransactions, client.name);

          return totalDebt > 0 || hasActualReturnableItems;
        }); // Show only clients with debt
    }

    // Sort results numerically by ID to prevent G1,G10,G100 ordering
    return results.sort((a, b) => {
      const aNum = parseInt(a.id.match(/G(\d+)/)?.[1] || '0');
      const bNum = parseInt(b.id.match(/G(\d+)/)?.[1] || '0');
      return aNum - bNum;
    });
  }, [searchedClients, showAllClients, getClientTotalDebt, getClientTransactions]);

  // Sort clients: maintain the order from context (which handles moveClientToFront)
  const sortedClients = useMemo(() => {
    // Use fast preview when clearing search for instant feedback
    const clientsToSort = isShowingFastPreview ? fastPreviewClients : filteredClients;
    return [...clientsToSort].sort(() => {
      // Don't sort by date - maintain the order from context to preserve moveClientToFront positioning
      return 0;
    });
  }, [filteredClients, fastPreviewClients, isShowingFastPreview]);
  ...
  const handleCalculatorInput = (value: string) => {
    ...
    // BUT NOT for CHECK buttons - they should still work
    if (autoReplayCompleted && value !== 'AC' && value !== 'CHECK→' && value !== 'CHECK←') {
      setAutoReplayDisplay('');
      setAutoReplayStepInfo(null);
      setAutoReplayCompleted(false);
    }
  ...
  // Function to refresh timeline scroll (restart from right)
  const handleRefreshTimeline = () => {
    const scrollingTabsElement = document.querySelector('.scrolling-tabs-component');
    if (scrollingTabsElement) {
      const timelineRef = (scrollingTabsElement as any).__timelineRef;
      if (timelineRef && timelineRef.current) {
        // Kill current timeline
        timelineRef.current.kill();

        // Dispatch event to restart timeline from beginning
        window.dispatchEvent(new CustomEvent('creditDataChanged', {
  ...
  const confirmDeleteAllClients = async () => {
    ...
    try {
      // Clear all credit data in localStorage (legacy support)
      localStorage.removeItem('creditClients');
      localStorage.removeItem('creditTransactions');
      localStorage.removeItem('creditPayments');

      // Clear all credit data in IndexedDB
      await creditDBManager.clearAllData();
        ...
        <div className="calculator-container w-full lg:w-[32rem] calculator-container-landscape bg-white rounded-lg shadow-lg p-4 lg:p-6 order-1 lg:order-2 flex flex-col h-full">
          ...
          <div className="mb-4">
            <div className="bg-black rounded-lg p-4 mb-2 calculator-display">
              {/* Main Display */}
              <div className="text-2xl sm:text-3xl font-mono text-green-400 min-h-[3rem] flex items-center overflow-x-auto bg-black rounded px-3 py-2 relative calculator-display">
                ...
                {/* Calculator Value - Full width with auto font-size scaling */}
                <div
                  className="text-right flex-1 flex items-center justify-end whitespace-nowrap pr-2"
                  title={autoReplayActive && autoReplayDisplay ? autoReplayDisplay : calculatorValue}
                  style={{
                    fontSize: (() => {
                      const displayValue = formatCalculatorValue((autoReplayActive || (autoReplayDisplay && autoReplayDisplay.startsWith('='))) && autoReplayDisplay ? autoReplayDisplay : calculatorValue);
                      const length = displayValue.length;
                      if (length <= 8) return 'inherit';
              ...
              {/* Secondary Display - with counter positioned below MU on left */}
              <div className="text-xs text-gray-400 font-mono mt-1 flex items-center relative calculator-secondary-display px-3 min-h-[2rem]">
                {/* Article Count Circle - Positioned to not overlap main display */}
                {(autoReplayActive && autoReplayStepInfo) || (autoReplayDisplay && autoReplayDisplay.startsWith('=')) || autoReplayStepInfo ? (
                  <div className="absolute left-1 top-0 bg-blue-500 text-white rounded-full w-7 h-7 flex items-center justify-center text-xs font-bold">
