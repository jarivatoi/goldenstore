...
export interface OrderItem {
  ...
  isVatNil: boolean; // Whether item is VAT exempt
  isVatIncluded: boolean; // Whether VAT is included in the price
  vatPercentage: number; // VAT percentage at time of order (snapshot)
  vatAmount: number; // VAT amount (vatPercentage of unit price * quantity)
...
export interface OrderReturn {
  ...
  baseAmount: number; // Base return amount
  vatPercentage: number; // VAT percentage applied
  vatAmount: number; // Calculated VAT amount
  totalReturnAmount: number; // Total return (base + VAT)
  createdAt: Date; // When return was added
  lastEditedAt?: Date; // When return was last modified
}
...
export interface Order {
  ...
  returns: OrderReturn[]; // Array of returns for this order
