...
export class KeypadHandler {
  ...
  public static processInput(
    ...
    switch (validation.type) {
      ...
      case 'equals':
        ...
        // Don't reformat display after equals - it's already properly formatted with trailing zeros
  ...
  private static handleNumberInput(state: KeypadState, input: string): KeypadState {
    ...
    // Treat '0.' as a new number start (for cases like "+.4")
    if (state.isNewNumber || state.display === '0' || state.display === '-0' || state.display === '0.')  {
    ...
    if (state.display === '-0') {
      newDisplay = '-' + input; // Prepend minus sign
    } else if (state.display === '0.') {
      newDisplay = '0.' + input; // Keep the decimal point (for cases like "+.4" → "0.4")
    } else {
      newDisplay = input; // Replace display with new input
    }
  ...
  private static handleDecimalInput(state: KeypadState): KeypadState {
    // If starting a new number, always start with "0." regardless of what's in display
    ...
    // For continuing to build the current number, check if it already has a decimal
  ...
  /**
   * Format display to show 2 decimal places if it has 1 decimal place
   */
  private static formatDisplayForOperator(display: string): string {
    // Check if display contains a decimal point
    if (display.includes('.')) {
      const parts = display.split('.');
      // If there's exactly 1 digit after decimal point, add a zero
  ...
  private static handleOperatorInput(state: KeypadState, input: string): KeypadState {
    ...
    // Format display to show 2 decimal places if it has 1 decimal place
    const formattedDisplay = this.formatDisplayForOperator(state.display);
    ...
    if (!isCompoundOperation && newCalculationSteps.length === 0 && currentNumber !== 0) {
        // Only create a step for the first number if it's non-zero
      const firstStep = {
      ...
      newArticleCount = 1;
    } else if (!isCompoundOperation && newCalculationSteps.length === 0 && currentNumber === 0) {
    }
    
    // Handle multiplication/division as compound operations
    if (normalizedOperator === '*' || normalizedOperator === '/' || normalizedOperator === '%') {
      // Special case: If we just finished a calculation (isNewNumber=true) and we're starting a new operation
      // with a different operator type, clear the calculation steps and start fresh
      if (state.isNewNumber && newCalculationSteps.length > 0 &&
          newCalculationSteps.some(step => step.isComplete)) {
        // Check if the last step is a completed operation (result of pressing =)
        // If so, clear the steps and start a new calculation
        newCalculationSteps = [];
        newArticleCount = 0;
      }
    ...
    if (normalizedOperator === '+' || normalizedOperator === '-') {
      ...
      if (state.isNewNumber && newCalculationSteps.length > 0 &&
          newCalculationSteps.some(step => step.isComplete) &&
          state.lastOperation && state.lastOperation !== normalizedOperator) {
        // Check if the last step is a completed operation (result of pressing =)
        // If so, clear the steps and start a new calculation
        newCalculationSteps = [];
        newArticleCount = 0;
      ...
      // Special case: Starting a negative number from scratch
      // When display is "0", no previous operations, and user presses minus
      if ((state.display === '0' || state.isNewNumber) &&
          normalizedOperator === '-' &&
          newCalculationSteps.length === 0 &&
          !state.lastOperation) {
        return {
          ...state,
          display: '-',
      ...
      if (state.lastOperation && (state.lastOperation === '+' || state.lastOperation === '-')) {
        ...
        if (newCalculationSteps.length === 0) {
          ...
          let resultDisplay = prevResult.toString();
          if (resultDisplay.includes('.')) {
            const parts = resultDisplay.split('.');
            if (parts[1] && parts[1].length === 1) {
              resultDisplay = resultDisplay + '0';
            }
          }

          return {
            ...state,
            display: resultDisplay, // Show the result of the previous operation
            lastOperation: normalizedOperator,
      ...
      if (state.lastOperation && (state.lastOperation === '*' || state.lastOperation === '/' || state.lastOperation === '%')) {
        ...
        if (newCalculationSteps.length === 0) {
          ...
          // Format the display to show 2 decimal places if it has 1
        ...
        if (!state.lastOperation && newCalculationSteps.length > 0 && 
          ...
          let displayValue = result.toString(); // Declare displayValue here
        ...
        // BUT ONLY if the number is not zero
        if (newCalculationSteps.length === 0) {
          // Get the first number from the state
          const firstNumber = state.lastOperand !== null ? state.lastOperand : this.getCurrentNumber(state.display);

          // Only create a step if we have a non-zero number
          if (firstNumber !== 0) {
            const firstStep: CalculationStep = {
              expression: firstNumber.toString(),
              result: firstNumber,
        ...
        // BUT only if we have existing steps or a non-zero number
        if (state.lastOperation !== null && (newCalculationSteps.length > 0 || currentNumber !== 0)) {
        ...
        return {
          ...
          calculationSteps: newCalculationSteps,
          nextOperatorContext: state.nextOperatorContext || normalizedOperator, // Preserve existing context or use new operator
          articleCount: newCalculationSteps.length // Use actual step count, not the carried-over value
        };
      }
    }
  ...
  private static handleEqualsInput(state: KeypadState): KeypadState {
    ...
    // Special case: Handle repeat equals operations (pressing = multiple times)
    // This happens when isNewNumber is true AND we have a lastOperation AND lastOperand
    // The display contains the previous result, and we want to repeat the same operation
    // BUT: We must have calculation steps already, otherwise this is the FIRST equals, not a repeat
    if (state.isNewNumber && state.lastOperation && state.lastOperand !== null && state.calculationSteps.length > 0) {
      // Perform the operation: currentNumber (previous result) OP lastOperand (second operand)
      const result = this.performBasicOperation(currentNumber, state.lastOperand, state.lastOperation);

      // Round the result to avoid floating-point precision issues
      const roundedResult = Math.round(result * 10000000000) / 10000000000;

      // Format the result to show 2 decimal places if it has 1
      ...
      return {
        ...state,
        display: resultStr,
        grandTotal: state.grandTotal + roundedResult,
        transactionHistory: [...state.transactionHistory, roundedResult],
        // Keep the same calculation steps - don't add new ones for repeat operations
        calculationSteps: state.calculationSteps,
        lastOperation: state.lastOperation, // Preserve operation for further repeats
        lastOperand: state.lastOperand, // Preserve the second operand for further repeats
        isNewNumber: true,
        isError: false,
        articleCount: state.articleCount // Keep the same article count
      };
    }
    ...
    if (state.lastOperation === 'PERCENT' && state.lastOperand !== null) {
      ...
      if (percentResultStr.includes('.')) {
        const parts = percentResultStr.split('.');
        if (parts[1] && parts[1].length === 1) {
          percentResultStr = percentResultStr + '0';
        }
      }

      return {
        ...state,
        display: percentResultStr,
    ...
    if (newCalculationSteps.length === 0 && !isStandaloneCompoundOperation) {
      ...
      // Only create a step for the first number if it's not zero
      if (firstNumber !== 0) {
        const firstStep: CalculationStep = {
          expression: firstNumber.toString(),
          result: firstNumber,
    ...
    if (state.lastOperation === '*' || state.lastOperation === '/' || state.lastOperation === '%') {
      ...
      // Only create a step if we have a non-zero result or existing steps
      if (currentNumber !== 0 || newCalculationSteps.length > 0) {
        const finalStep: CalculationStep = {
          expression: `${state.lastOperation}${currentNumber}`,
          result: currentNumber,
          timestamp: Date.now(),
    ...
    // If no meaningful steps were created and the result is 0, don't process the equals
    if (newCalculationSteps.length === 0 && finalResult === 0) {
      return state;
    }
    ...
    // For the first equals, preserve currentNumber as the second operand for future repeat operations
    // This is always the correct value since currentNumber comes from the display (the second operand the user typed)
    let operandToPreserve = currentNumber;

    // Round the final result to avoid floating-point precision issues
    // Round to 10 decimal places first to handle precision errors, then display appropriately
    const roundedResult = Math.round(finalResult * 10000000000) / 10000000000;

    // Format the final result display
    let finalResultDisplay = roundedResult.toString();
    if (finalResultDisplay.includes('.')) {
      const parts = finalResultDisplay.split('.');
      if (parts[1] && parts[1].length === 1) {
        finalResultDisplay = finalResultDisplay + '0';
      }
    }

    // Update finalResult to use the rounded value
    ...
    const newState = {
      ...
      lastOperand: shouldPreserveOperation ? operandToPreserve : null,
      isNewNumber: true,
      isError: false,
      articleCount: newCalculationSteps.length
    };
  ...
  private static handleSpecialFunction(
    ...
    switch (input) {
      ...
      case '+/-':
        ...
        // Special case: if display is '-0', change it to '0'
        if (state.display === '-0') {
          return {
            state: { ...state, display: '0', isNewNumber: false },
            isActive: true
          };
        }
    ...
    setTimeout(() => {
      if (state.calculationSteps.length > 1) {
        ...
        setTimeout(() => {
          ...
          let resultStr = finalResult.toString();
  ...
  private static handleCheckForward(state: KeypadState): KeypadState {
    ...
    if (hasResult && currentIndex === state.calculationSteps.length) {
      // Show result - special handling with currentStep: -1
      ...
      // Dispatch event to update UI - pass -1 as currentStep marker for result
      window.dispatchEvent(new CustomEvent('checkNavigation', {
        detail: {
          displayValue: displayValue,
          stepIndex: currentIndex,
          totalSteps: state.calculationSteps.length, // Don't include result in total
          currentStep: -1, // Special marker for result
      ...
      // Dispatch event to update UI with step counter
      window.dispatchEvent(new CustomEvent('checkNavigation', {
        detail: {
          displayValue: displayValue,
          stepIndex: currentIndex,
